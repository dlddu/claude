...
워크플로우 결과 필드와 subagent 출력을 조합한 JSON을 임시 파일에 저장한 후, 파일 경로를 인수로 전달하여 스크립트를 실행합니다:

```bash
# 1. JSON을 임시 파일에 저장
echo '{script_input}' > /tmp/linear-status-input.json

# 2. 스크립트 실행 (DEBUG=1로 상세 로그 활성화)
DEBUG=1 {skill_directory}/scripts/linear-status-report.sh /tmp/linear-status-input.json
```
...
- 스크립트에 입력 파일 인수가 없거나 파일이 존재하지 않는 경우: `error_stage`가 `init`이며 `error`에 파일 관련 메시지가 포함됩니다.
