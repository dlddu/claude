---
argument-hint: <sub-task-id>
description: Linear sub-task에 대한 작업을 수행하고 상태를 업데이트합니다
...
1. Linear MCP를 사용하여 sub-task 정보를 가져옵니다
...
Linear MCP를 사용하여 sub-task 상태를 **Done**으로 변경합니다.
...
Blocked 상태로 변경 시 반드시 다음을 sub-task에 코멘트로 남깁니다:
- 블로킹 사유
- 해결을 위해 필요한 조치
- 관련된 이슈나 담당자 (알고 있는 경우)
